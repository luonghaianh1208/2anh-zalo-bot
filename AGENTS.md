...
3. Run `npm ci` in this repository.
4. Run `npm run install:hermes -- --hermes-home <resolved-path>`.
5. Run `npm run doctor -- --hermes-home <resolved-path>` and report every failed check.
6. If doctor passes, tell the user to run `npm start`, open `http://127.0.0.1:3872`, scan the QR code with a secondary Zalo account, and configure their owner UID.
