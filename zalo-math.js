/**
 * Đổi công thức LaTeX trong tin bot gửi sang ký tự Unicode trước khi ra Zalo.
 *
 * Zalo không hiển thị LaTeX, nên "$Ca^{2+}$" hiện nguyên dấu đô la và mũ. Ở đây
 * đổi thành "Ca²⁺", "\rightarrow" thành "→", "\alpha" thành "α"… Chỉ đụng vào
 * chỗ chắc chắn là công thức: trong $…$, $$…$$, \(…\), \[…\]; lệnh LaTeX có dấu
 * "\" đứng trần; mũ/chỉ số dạng hoá học (H_2O, x^2). Mã trong `…` / ``` … ```,
 * tên biến kiểu snake_case và giá tiền "$5 và $10" giữ nguyên.
 */
...
function convertMath(expr) {
  ...
  // Như LaTeX: dấu cách sau lệnh chữ cái (Hy Lạp, ∂, ∞…) chỉ để kết thúc lệnh — "\Delta H" là ΔH.
...
function convertProse(text) {
  ...
  out = replaceSymbols(out);
  // H_2O, CO_2, SO_{4}: chỉ sau ký hiệu hoá học viết hoa, để snake_case như file_2 giữ nguyên.
  // Chạy trước mũ để "SO_{4}^{2-}" còn chữ số ngay trước dấu mũ.
  out = out.replace(/(?<=(?:^|[^\p{L}\p{N}_])(?:[A-Z][a-z]?)+)_(\{[^{}\s]*\}|\d+)(?![\p{L}_])/gu, (_, part) => (
    script(part.replace(/^\{|\}$/g, ''), SUBSCRIPT, '_')));
  // x^2, Ca^{2+}, K^+ viết trần: chỉ khi ngay trước là chữ/số/ngoặc đóng.
...
export function latexToUnicode(text) {
  ...
  return source
    // Mã và đường link giữ nguyên: "?x=1^2" trong URL không phải công thức.
    .split(/(```[\s\S]*?```|`[^`\n]*`|https?:\/\/\S+)/)
